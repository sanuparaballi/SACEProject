...
class HyperRepresentation(BilevelProblem):
    ...
    def __init__(self, n=100, m=10, d2=2):
        ...
        self.X_train = np.random.randn(n, m)
        self.Y_train = np.random.randn(n)
        self.X_val = np.random.randn(n, m)
        self.Y_val = np.random.randn(n)
    ...
    def evaluate(self, ul_vars, ll_vars, add_penalty=True):
        ...
        """
        # Reshape leader's variables back into a matrix
        A = ul_vars.reshape((self.ll_dim, self.ll_dim))
        w = ll_vars
